'use client';

export default function ToggleSwitch({ label, defaultChecked = false, onChange = () => { } }: { label?: string, defaultChecked?: boolean, onChange?: (checked: boolean) => void }) {
    return (
        <label className="relative inline-flex items-center cursor-pointer text-gray-900 gap-3">
            <input defaultChecked={defaultChecked} onChange={(e) => onChange(e.target.checked)} type="checkbox" className="sr-only peer" />
            <div className="w-10 h-5 bg-slate-300 rounded-full peer peer-checked:bg-indigo-600 transition-colors duration-200"></div>
            <span className="dot absolute left-1 top-1 w-3 h-3 bg-white rounded-full transition-transform duration-200 ease-in-out peer-checked:translate-x-5"></span>
            {label}
        </label>
    )
}
